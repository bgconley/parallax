...
- Keep active Parallax UI scope temporal: timing sessions, timing launcher/calibration, timing review/correction, checkpointed timing only as temporal instrumentation, temporal home/current focus, offline/sync/AI-pending/needs-review/high-contrast/Dynamic Type/reduced-motion states for those workflows, and grounded temporal answers only where Phase 7 supports them.
- Do not add broad task/project management, generic inbox/task-tracker flows, routine builders, broad weekly reviews, broad personalization/settings, "break it down" task decomposition, step-planning language, or unrelated assistant/planning workflows just because they appear in the expansion pack or Figma reference material. Require a canonical artifact or explicit product decision first.
- Active Figma work must be source-backed or canonical-derived from `parallax_v1_3_artifact_pack/examples/` and the current canonical Figma reference pages. Current canonical visual references include `https://www.figma.com/design/OYOtLrgwZmqAqsURzYJBM9/Parallax---v1.3-Native-Screens?node-id=118-2` and `https://www.figma.com/design/OYOtLrgwZmqAqsURzYJBM9/Parallax---v1.3-Native-Screens?node-id=85-2`. Treat those frames as pixel-level design authority only: layout, spacing, typography, component proportions, visual hierarchy, and affordances. Do not hard-code dummy/example content, user names, activity names, canned rows, task/project concepts, or mock workflow examples into runtime code; runtime content must come from canonical Parallax workflows, local state, user-entered data, and API/view-model projections. Node `118:2` is a mixed board that includes unfinished mocks, hotspots, and Phase 10 target material; use only complete canonical frames such as board `118:3` for current Phase 9 visual alignment unless the user explicitly opens later scope.
- Simplified vector drafts are not finished mockups.
...
- Current Phase 9 polish handoff: Timing Session bottom pull-up drawer/dock is fixed and simulator-verified as of 2026-05-06, but the broader app-wide polish audit remains active. Do not accept `PQ15-timing-session-card-polish.jpg`, `PQ16-timing-session-anchored-dock-and-secondary-buttons.jpg`, `PQ29-timing-session-bottom-drawer-attached-v2.jpg`, or `PQ30-dynamic-type-timing-session-bottom-drawer-attached-v2.jpg` as final proof; those are rejected/intermediate. Accepted Timing Session proof is `PQ31-dynamic-type-timing-session-bottom-drawer-attached-v3.jpg` and `PQ32-timing-session-bottom-drawer-attached-v3.jpg`.
- Accepted code state for that issue: `TimingSessionScreen` uses a bottom `ZStack` overlay rather than a root `VStack`, reserves scroll content behind `bottomActionDock`, uses compact vertical system-bordered secondary buttons, gives the dock a shared flat-bottom sheet shape, and keeps the duplicate step-preview `Add run note` action removed because the main `Note` control already exists.
- Bottom-attached dock implementation rule: use a bottom overlay with reserved scroll space, shared flat-bottom sheet shape, bottom safe-area fill, and a downward safe-area attachment offset. Do not rely on bottom padding alone; it can still leave a visible bottom canvas band in a safe-area-bounded parent. Timing Review now uses the same attached pattern (`PQ33-review-bottom-drawer-attached-v2.jpg`), and terminal drawer actions such as Close/Cancel suppress drill-in chevrons (`PQ36`, `PQ37`).
