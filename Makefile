.PHONY: validate test lint typecheck security release-status release-snapshot-deployment release-promote-deployment-check release-preflight release-gate release-slo privacy-log-scan privacy-lifecycle-smoke backup-restore-drill migrate schema-smoke phase1-smoke phase2-smoke phase3-smoke phase4-smoke phase5-smoke phase6-smoke phase7-smoke phase8-smoke phase9-smoke phase10-smoke dev-up dev-down dev-logs
...
release-promote-deployment-check:
	test -n "$(RELEASE_DEPLOYMENT_SNAPSHOT)"
...
release-gate:
	...
	uv run python scripts/record_release_gate.py --gate performance_slo --proof-dir "$(RELEASE_PROOF_DIR)" -- uv run python scripts/release_slo_smoke.py --api-url "$(PARALLAX_API_URL)" --bearer-token "$$PARALLAX_RELEASE_BEARER_TOKEN" --app-check-token "$$PARALLAX_RELEASE_APP_CHECK_TOKEN" --require-bearer-auth
	uv run python scripts/record_release_gate.py --gate production_log_privacy_scan --proof-dir "$(RELEASE_PROOF_DIR)" -- uv run python scripts/release_log_privacy_scan.py --api-url "$(PARALLAX_API_URL)" --bearer-token "$$PARALLAX_RELEASE_BEARER_TOKEN" --app-check-token "$$PARALLAX_RELEASE_APP_CHECK_TOKEN" --require-bearer-auth
	uv run python scripts/record_release_gate.py --gate backup_restore --proof-dir "$(RELEASE_PROOF_DIR)" -- uv run python scripts/release_backup_restore_drill.py --database-url "$(PARALLAX_HOST_DATABASE_URL)" --object-root "$${PARALLAX_OBJECTS_DIR:-/srv/parallax/objects}" --restore-root "$${PARALLAX_RESTORE_DRILL_ROOT:-/srv/parallax/exports/release-restore-drill}"
